...
from settings import IMAGES_ON_PAGE
...
class DBManager(metaclass=SingletonMeta):
    ...
    def get_images(self, page: int = 1) -> list[tuple]:
        """Get images to show on chosen page"""
        offset = (page - 1) * IMAGES_ON_PAGE
        logger.info(f"Try to get images with offset {offset}")
        with self.connect().cursor() as cursor:
            cursor.execute("SELECT * FROM images ORDER BY upload_time DESC LIMIT %s OFFSET %s", (IMAGES_ON_PAGE, offset))
    ...
    def clear_images(self) -> None:
        """Clear all uploaded images from db"""
        with self.conn.cursor() as cursor:
